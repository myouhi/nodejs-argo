...
const UUID = process.env.UUID || '28807da0-0131-45bb-8269-a8974a083966'; // 使用哪吒v1,在不同的平台运行需修改UUID,否则会覆盖
...
app.get("/", (req, res) => {
  ...
  if (fs.existsSync(indexPath)) {
      ...
      res.send("Welcome! Service is running. Please place index.html in the 'public' folder.");
  }
});
  ...
  if (fs.existsSync(bookmarksPath)) {
    fs.readFile(bookmarksPath, 'utf8', (err, data) => {
      ...
      try {
        ...
        res.status(500).json({ error: 'Failed to parse bookmarks file.' });
      }
    });
  } else {
